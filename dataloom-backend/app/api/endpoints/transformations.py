...
def _handle_basic_transform(df, transformation_input, project, db, project_id):
    ...
    if op == "filter":
        ...
        if p.criteria:
            criteria_list = [{"column": c.column, "ascending": c.ascending} for c in p.criteria]
            return ts.apply_sort(df, criteria=criteria_list), True

        # Single-column sorting (backward compatible)
        return ts.apply_sort(df, column=p.column, ascending=p.ascending), True
